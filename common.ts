...
import { readableStreamToString, CoalesceError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new CoalesceError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new CoalesceError(e, e.response?.data)
                }
                if (innerError instanceof CoalesceError) {
                    // Got "CoalesceError" from the above try block
                    throw innerError;
                }
